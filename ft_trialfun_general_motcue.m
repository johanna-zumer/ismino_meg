function [trl, event] = ft_trialfun_general_motcue(cfg)
...
for i=sel
  ...
  if trlbeg>0 && trlend<=hdr.nSamples*hdr.nTrials,
    ...
    trlcnt=trlcnt+1;
    try
      nexti=sel(find(sel==i)+1);
    catch % end of loop of trials
      if sel(end)==i
        nexti=length(event);
      else
    ...
    for ind = i+1:nexti
      if strcmp(event(ind).type,cfg.trialdef.eventtype)
        if ~isempty(intersect(event(ind).value,[31 32])) % aud move (right/left)
          addval(trlcnt,1)=event(ind).value;
        end
        if ~isempty(intersect(event(ind).value,[41 42 40])) % vis move (right/left/absent)
          addval(trlcnt,2)=event(ind).value;
        end
      end
      if strcmp(event(ind).type,'UPPT001')    % response channel
%         if ~isempty(intersect(event(ind).value,[104 122 107 109])) % leftcert / leftuncert / rightuncert / rightcert
        if ~isempty(intersect(event(ind).value,[1 2 3 8])) % leftcert / leftuncert / rightuncert / rightcert
          addval(trlcnt,3)=event(ind).value;
          if [addval(trlcnt,3)==1 || addval(trlcnt,3)==2] && addval(trlcnt,1)==32
            addval(trlcnt,4)=1; % correct 
          end
          if [addval(trlcnt,3)==3 || addval(trlcnt,3)==8] && addval(trlcnt,1)==31
            addval(trlcnt,4)=1; % correct
          end
        end
      end
...
addval=[addval addval(:,2)-addval(:,1)==10];
% addval: [aud_dir vis_dir/abs response response_corr AVcongruent]
